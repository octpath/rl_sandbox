...
class MCTS:
    ...
    def __init__(self, model, c_puct=1.0, epsilon=0.25):
        self.model = model
        self.c_puct = c_puct
    ...
    def search(self, root_state, current_player, num_simulations, game, alpha=None):
        ...
        for _ in range(num_simulations):
            # PUCTに基づいて行動を選択
            action, next_state = self._select_action_and_next_state(root_state, current_player, game)
    ...
    def _evaluate(self, state, current_player, game):
        ...
        if game.is_done(state):
            ...
            action, next_state = self._select_action_and_next_state(state, current_player, game)
            v = -self._evaluate(next_state, -current_player, game)
            self.W[s][action] += v
            self.N[s][action] += 1
            return v
    ...
    def _expand(self, state, current_player, game):
        ...
        # 次に取り得る状態を得る (リーフの展開)
        self.next_states[s] = [
            (game.step(state, action, current_player)[0] if (action in legal_moves) else None)
    ...
    def _select_action_and_next_state(self, state, current_player, game):
        ...
        scores = np.array([score if action in legal_moves else -np.inf for action, score in enumerate(scores)])
